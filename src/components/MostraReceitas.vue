...
import IReceita from "@/interfaces/IReceita.vue";
import CardReceita from "@/components/CardReceita.vue";
import { obterReceitas } from "@/http/index.vue";
import { itensDeLista1EstaoEmLista2 } from "@/operacoes/listas.vue";
...
export default {
  ...
  async created(){
    ...
    this.receitasEncontradas = await receitas.slice(0,8);

        receitas.filter((receita) => {
      const possoFazerReceita = itensDeLista1EstaoEmLista(receita.ingredientes, this.ingredientes);



      console.log("aqui -> ", receita.ingredientes);
      console.log("aqui ,,,-> ", this.ingredientes);
